...
/**
 *    \file       clienjoyholidays_list.php
 *        \ingroup    clienjoyholidays
 *        \brief      List page for clienjoyholidays
 */
...
// load clienjoyholidays libraries
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'clienjoyholidayslist'; // To manage different context of search
...
$hookmanager->initHooks(array('clienjoyholidayslist')); // Note that conf->hooks_modules contains array
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'CliEnjoyHolidays';
	$objectlabel = 'CliEnjoyHolidays';
...
// --------------------------------------------------------------------
$sql = 'SELECT ';
$sql .= $object->getFieldListRetrocompatibility('t');
...
if ($num == 1 && getDolGlobalString('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: " . dol_buildpath('/clienjoyholidays/clienjoyholidays_card.php', 1) . '?id=' . $id);
	exit;
}
...
if ($socid > 0 && $user->hasRight('societe', 'lire')) {
	$socstat = new Societe($db);
	$res = $socstat->fetch($socid);
	if ($res > 0) {
		$head = societe_prepare_head($socstat);

		print dol_get_fiche_head($head, 'tabCliEnjoyHolidays', $langs->trans("ThirdParty"), -1, 'company');
...
$objecttmp = new CliEnjoyHolidays($db);
$trackid = 'xxxx' . $object->id;
include DOL_DOCUMENT_ROOT . '/core/tpl/massactions_pre.tpl.php';

//TODO Mass action from clienjoyholidays list to propal lines
if ($massaction == 'preaddpropal') {
	...
	print '<h4>' . $langs->trans('CEHMassActionValidation') . '</h4>';
...
if ($massaction == 'preaddtasktoproject') {
	...
	print '<input type="hidden" name="token" value="' . newToken() . '" />';
	print '<div class="select-mass-action-container warning"  >';
	print '<h4>' . $langs->trans('CEHMassActionValidationProject') . '</h4>';
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_clienjoyholidays', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}
